...
pub(crate) use meta::{get_message, get_meta};
...
fn map_err(err: impl ToString) -> anyhow::Error {
...
fn new_session(data: &ServerState, result: ProcessCommitsResult) -> Result<CommitResponse> {
    let session = if !result.continue_.is_empty() {
        let session = SessionId(random());
...
pub(crate) async fn get_commits(
    Path(refname): Path<String>,
    State(data): State<Arc<ServerState>>,
) -> std::result::Result<Json<Value>, AnyhowError> {
    let time_load = Instant::now();
    ...
    Ok(Json(json!(new_session(&data, result)?)))
}
